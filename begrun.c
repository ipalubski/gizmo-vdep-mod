...
#define MAXTAGS 300
  ...
  if(ThisTask == 0)		/* read parameter file on process 0 */
...
#ifdef SHEAPOTENTIAL
      strcpy(tag[nt], "PotentialScaleFile");
      addr[nt] = &potential.scalefile;
      id[nt++] = STRING;
#endif

#ifdef FIXEDSHEAPOTENTIAL
      strcpy(tag[nt], "PotentialX");
      addr[nt] = &potential.X;
      id[nt++] = REAL;

      strcpy(tag[nt], "PotentialY");
      addr[nt] = &potential.Y;
      id[nt++] = REAL;

      strcpy(tag[nt], "PotentialZ");
      addr[nt] = &potential.Z;
      id[nt++] = REAL;
#endif
    ...
    if(ThisTask == 0)
      {  //only task 0 reads in the file                                                                             
	if(!(mfile = fopen(potential.scalefile, "r")))
	  {
	    printf("error in opening file '%s'\n", potential.scalefile);
	    endrun(1);
	  }
	printf("Reading in parameters for the potential from %s\n",potential.scalefile);
	int i = 0;
	for(i = 0; i < sizeof(potential.TimeArray)/sizeof(double); i++)      //Zero out the arrays                  
	  {
	    /* NEED TO ADD FLAG FOR SHEAPOTENTIAL = 3 */
#if !(SHEAPOTENTIAL==3)
	    potential.TimeArray[i] = -1;  potential.MassArray[i] = -1;  potential.RadiusArray[i] = -1;
#if (SHEAPOTENTIAL==1) || (SHEAPOTENTIAL == 2)
	    potential.HeightArray[i] = -1;
#endif
#endif
	    
#if (SHEAPOTENTIAL==3)
	    potential.TimeArray[i] = -1;
	    potential.MassStarsArray[i] = -1;  potential.RadiusStarsArray[i] = -1; potential.HeightStarsArray[i] = -1;
	    potential.MassGasArray[i] = -1;  potential.RadiusGasArray[i] = -1; potential.HeightGasArray[i] = -1;
	    potential.MassBulgeArray[i] = -1;  potential.RadiusBulgeArray[i] = -1;
#endif
	  }
	i = 0;
#if (SHEAPOTENTIAL==0)
	while(fscanf(mfile,"%lf %lf %lf",&potential.TimeArray[i],&potential.MassArray[i],&potential.RadiusArray[i])!= EOF)   //Read in the data from a file
#elif (SHEAPOTENTIAL==1) || (SHEAPOTENTIAL==2)
	  while(fscanf(mfile,"%lf %lf %lf %lf",&potential.TimeArray[i],&potential.MassArray[i],&potential.RadiusArray[i],&potential.HeightArray[i]) != EOF)   //Read in the data                                                    
#elif (SHEAPOTENTIAL==3)
	    while(fscanf(mfile,"%lf %lf %lf %lf %lf %lf %lf %lf %lf",&potential.TimeArray[i],&potential.MassStarsArray[i],&potential.MassGasArray[i],&potential.MassBulgeArray[i],&potential.RadiusStarsArray[i],&potential.RadiusGasArray[i],&potential.RadiusBulgeArray[i],&potential.HeightStarsArray[i],&potential.HeightGasArray[i]) != EOF)
#else
	      printf("Don't know any other values for the potential.  Exiting.\n");
	endrun(1);
#endif
	{ i++; }    // the while loop that reads the file                                                        
	potential.NumSteps = i;
      }    //closes check of Task0                                                                                   
    
    if(ThisTask == 0)
      {
        printf("\nRead in %d times, masses, etc. from %s for the potential\n",potential.NumSteps,potential.scalefile);
        for(i = 0; i < potential.NumSteps; i++)
	  {
#if (SHEAPOTENTIAL==1) || (SHEAPOTENTIAL==2)
            printf("t = %g, M = %g, Rd = %g, Zd = %g\n",potential.TimeArray[i],potential.MassArray[i],potential.RadiusArray[i],potential.HeightArray[i]);
#elif (SHEAPOTENTIAL==0)
            printf("t = %g, M = %g, Rd = %g\n",potential.TimeArray[i],potential.MassArray[i],potential.RadiusArray[i]);
#elif (SHEAPOTENTIAL==3)
            printf("t = %g, Md = %g, Rd = %g, Zd = %g, Mg = %g, Rg = %g, Zg = %g, Mb = %g, Rb = %g\n",potential.TimeArray[i],potential.MassStarsArray[i],potential.RadiusStarsArray[i],potential.HeightStarsArray[i],potential.MassGasArray[i],potential.RadiusGasArray[i],potential.HeightGasArray[i],potential.MassBulgeArray[i],potential.RadiusBulgeArray[i]);
#endif
	  }
      }
    /* initialize the potential the first time */
    update_potential_properties();
    
    /* and finally broadcast the structure, which now has the number of steps, the lookup tables, and the current properties */
    MPI_Bcast(&potential, sizeof(struct potential), MPI_BYTE, 0, MPI_COMM_WORLD);
#endif 
