...
#ifdef SHEAPOTENTIAL
        update_potential_properties();      /* scale the potential to this timestep */
#endif
    }
...
#ifdef SHEAPOTENTIAL
/*! This routine linearly interpolates y = f(x) at x = targetx, given                                                                                                                                        
 *  an x0 and x1 (where x0 < targetx < x1), f(x0) = y0, and f(x1) = y1 */
double lininterp( double targetx, double x0, double x1, double y0, double y1)
{ return y0 + (y1 - y0)*(targetx - x0)/(x1 - x0); }

/* This routine updates the Mass, Radius, and (if disk) Height of the potential                                                                                                                             
 * for this All.Time.  Should work regardless of cosmological or not, but the                                                                                                                               
 * time array (potential.TimeArray) MUST be sorted */
void update_potential_properties(void)
{
  // in principle, could use GSL to do smarter interpolation (e.g. with gsl_spline or gsl_interp), but should build those once, then call them later                                                      
  // instead, let's just do a simple lookup and linearly interpolate between the closest two                                                                                                              
  int ii = 0;
  for(ii = 0; ii < potential.NumSteps; ii++)
    {
      if(potential.TimeArray[ii] > All.Time)  // assumes the TimeArray is sorted                                                                             
	break;
    }
  // so All.Time is between potential.TimeArray[ii] and potential.TimeArray[ii-1]                                                                                    
  if(ii == 0 || ii == potential.NumSteps - 1)
    {
      // then I haven't even hit the first scaling given, or I'm past the end                                                                                          
      // either way, just use the first or last (real) entry in the arrays                                                                                                      
#if !(SHEAPOTENTIAL==3)
      potential.Mass = potential.MassArray[ii];
      potential.Radius = potential.RadiusArray[ii];
#if (SHEAPOTENTIAL==1) || (SHEAPOTENTIAL==2)
      potential.Height = potential.HeightArray[ii];
        #endif
#endif /* ends !(SHEAPOTENTIAL==3) */

#if (SHEAPOTENTIAL==3)
      potential.MassStars = potential.MassStarsArray[ii];
      potential.RadiusStars = potential.RadiusStarsArray[ii];
      potential.HeightStars = potential.HeightStarsArray[ii];

      potential.MassGas = potential.MassGasArray[ii];
      potential.RadiusGas = potential.RadiusGasArray[ii];
      potential.HeightGas = potential.HeightGasArray[ii];

      potential.MassBulge = potential.MassBulgeArray[ii];
      potential.RadiusBulge = potential.RadiusBulgeArray[ii];
        #endif
    }       // closes the check if I'm outside the times given
  else
    {
#if !(SHEAPOTENTIAL==3)
      //otherwise, linearly interpolate between the two nearest times given                                                                                                  
      potential.Mass = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.MassArray[ii-1], potential.MassArray[ii]);
      potential.Radius = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.RadiusArray[ii-1], potential.RadiusArray[ii]);
#if (SHEAPOTENTIAL==1) || (SHEAPOTENTIAL==2)
      potential.Height = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.HeightArray[ii-1], potential.HeightArray[ii]);
        #endif
#endif /* ends !(SHEAPOTENTIAL==3) */

#if (SHEAPOTENTIAL==3)
      potential.MassStars = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.MassStarsArray[ii-1], potential.MassStarsArray[ii]);
      potential.RadiusStars = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.RadiusStarsArray[ii-1], potential.RadiusStarsArray[ii]);
      potential.HeightStars = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.HeightStarsArray[ii-1], potential.HeightStarsArray[ii]);

      potential.MassGas = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.MassGasArray[ii-1], potential.MassGasArray[ii]);
      potential.RadiusGas = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.RadiusGasArray[ii-1], potential.RadiusGasArray[ii]);
      potential.HeightGas = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.HeightGasArray[ii-1], potential.HeightGasArray[ii]);

      potential.MassBulge = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.MassBulgeArray[ii-1], potential.MassBulgeArray[ii]);
      potential.RadiusBulge = lininterp(All.Time, potential.TimeArray[ii-1], potential.TimeArray[ii], potential.RadiusBulgeArray[ii-1], potential.RadiusBulgeArray[ii]);
        #endif
    }
  // if(ThisTask==0)                                                                                                                                            
  //   printf("SHEA:  Scaling the potential to M = %g, Rd = %g, Zd = %g at t = %g (interpolating between t = %g and t = %g)\n",potential.Mass,potential.Radius,potential.Height,All.Time,potential.TimeArray[ii-1],potential.TimeArray[ii]);                                                                                                                                 

}
#endif /* ends SHEAPOTENTIAL */
