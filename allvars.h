...
#define SIGFACTOR_TABLE_LENGTH 5000
extern MyDouble GeoFactorTable[GEOFACTOR_TABLE_LENGTH];
extern MyDouble SigFactorTable[SIGFACTOR_TABLE_LENGTH];
#endif

/* < SheaGK edits adding an analytic potential whose properties change with time */
#ifdef SHEAPOTENTIAL
extern struct potential  /* Need Mass, Radius, and Height if it's a disk; also need to store arrays of those, and I need a place to store the name of the file that I read the arrays from */
{
#if !(SHEAPOTENTIAL==3)
  MyDouble Mass;  /* < the instantenous mass of the potential */
  MyDouble Radius;  /* < the instantenous mass of the potential */
  MyDouble MassArray[1000];      /*! < A list that holds the mass of the potential at different times, in system units */
  MyDouble RadiusArray[1000];       /*! < A list that holds the scale radius of the potential at different times, in system units */
#endif
  
  int NumSteps;                 /*! < An integer that tells you the number of timesteps in the file that's read in, to only loop over those entries in the array */
  MyDouble TimeArray[1000];       /*! < A list that holds the times that the masses and radii are given at, in system units */
  
  char scalefile[500];  /*! < A 3 column file that holds the time, mass, and radius of the potential to be read in */
  
#if (SHEAPOTENTIAL == 1) || (SHEAPOTENTIAL == 2)
  MyDouble Height;
  MyDouble HeightArray[1000];
#endif
  
  /* TylerK edit allowing for multiple potentials */
#if (SHEAPOTENTIAL==3)
  MyDouble MassStars;
  MyDouble MassGas;
  MyDouble MassBulge;
  MyDouble RadiusStars;
  MyDouble RadiusGas;
  MyDouble RadiusBulge;
  MyDouble HeightStars;
  MyDouble HeightGas;

  MyDouble MassStarsArray[1000];
  MyDouble MassGasArray[1000];
  MyDouble MassBulgeArray[1000];
  MyDouble RadiusStarsArray[1000];
  MyDouble RadiusGasArray[1000];
  MyDouble RadiusBulgeArray[1000];
  MyDouble HeightStarsArray[1000];
  MyDouble HeightGasArray[1000];

#endif

  /* TylerK edit allowing for a fixed potential */
