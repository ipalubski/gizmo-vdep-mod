...
void GravAccel_GrowingiskPotential(void);
...
void accel(double M[3],double a[3],double b);
//double MNa(double z);
//double MNmass(double m,double z,double r);
void GravAccel_GrowingDiskplusHerquistPotential(void);
...
void add_analytic_gravitational_forces()
{
    ...
    GravAccel_GrowingDiskplusHerquistPotential();
#endif
...
#ifdef SHEAPOTENTIAL=3
void GravAccel_GrowingDiskplusHerquistPotential()
{
  double r2, r, f, m; int i, k;
  /* Compute masses and scale radii for the 3 MN disks */
  /* total mass and scale-length "a" for Hernquist potential [both in code units] */
  /* Compute gas mass and scale radius */
  double k1[6] = {-0.0090, 0.0173, -0.0051, -0.0358, -0.0830, -0.0247};
  double k2[6] = {0.0640, -0.0903, 0.0287, 0.2610, 0.4992, 0.1718};
  double k3[6] = {-0.1653, 0.0877, -0.0361, -0.6987, -0.7967, -0.4124};
  double k4[6] = {0.1164, 0.2029, -0.0544, -0.1193, -1.2966, -0.5944};
  double k5[6] = {1.9487, -1.3077, 0.2242, 2.0074, 4.4441, 0.7333};
  double NMmstar[3];
  double astar[3];
  double NMmgas[3];							
  double agas[3];

  double expRatio = potential.HeightStars / potential.RadiusStars;
  double gexpRatio = potential.HeightGas / potential.RadiusGas;
  double m_stars = potential.MassStars;
  double z_stars = potential.HeightStars;
  double r_stars = potential.RadiusStars;
  double m_gas = potential.MassGas;
  double r_gas = potential.RadiusGas;
  double z_gas = potential.HeightGas;

  double HQ_Mtot=0.22*m_stars, HQ_a=r_stars/5;
  double logM = log10(m_stars);
  //double fg = 2.22624 - 0.1804 * logM + 0.07319 * (1/t-1);
  //double m_gas = (m_disk + HQ_Mtot)*fg/(1-fg);
  //double r_gas = r_disk*2.8;
  double gMNS = (-0.269 * gexpRatio * gexpRatio * gexpRatio) + (1.080 * gexpRatio * gexpRatio) + (1.092 * gexpRatio);
  double gMNS2 = gMNS * gMNS;
  double gMNS3 = gMNS2 * gMNS;
  double gMNS4 = gMNS3 * gMNS;

  double MNS = (-0.269 * expRatio * expRatio * expRatio) + (1.080 * expRatio * expRatio) + (1.092 * expRatio);
  double MNS2 = MNS * MNS;
  double MNS3 = MNS2 * MNS;
  double MNS4 = MNS3 * MNS;

  double b_gas= gMNS * potential.RadiusGas;
  double b_stars= MNS * potential.RadiusStars;

  NMmstar[0] = (k1[0]*MNS4 + k2[0]*MNS3 + k3[0]*MNS2 + k4[0]*MNS + k5[0]) * m_stars;
  NMmstar[1] = (k1[1]*MNS4 + k2[1]*MNS3 + k3[1]*MNS2 + k4[1]*MNS + k5[1]) * m_stars;
  NMmstar[2] = (k1[2]*MNS4 + k2[2]*MNS3 + k3[2]*MNS2 + k4[2]*MNS + k5[2]) * m_stars;
  astar[0]  = (k1[3]*MNS4 + k2[3]*MNS3 + k3[3]*MNS2 + k4[3]*MNS + k5[3]) * r_stars;
  astar[1]  = (k1[4]*MNS4 + k2[4]*MNS3 + k3[4]*MNS2 + k4[4]*MNS + k5[4]) * r_stars;
  astar[2]  = (k1[5]*MNS4 + k2[5]*MNS3 + k3[5]*MNS2 + k4[5]*MNS + k5[5]) * r_stars;
  NMmgas[0] = (k1[0]*gMNS4 + k2[0]*gMNS3 + k3[0]*gMNS2 + k4[0]*gMNS + k5[0]) * m_gas;
  NMmgas[1] = (k1[1]*gMNS4 + k2[1]*gMNS3 + k3[1]*gMNS2 + k4[1]*gMNS + k5[1]) * m_gas;
  NMmgas[2] = (k1[2]*gMNS4 + k2[2]*gMNS3 + k3[2]*gMNS2 + k4[2]*gMNS + k5[2]) * m_gas;
  agas[0]  = (k1[3]*gMNS4 + k2[3]*gMNS3 + k3[3]*gMNS2 + k4[3]*gMNS + k5[3]) * r_gas;
  agas[1]  = (k1[4]*gMNS4 + k2[4]*gMNS3 + k3[4]*gMNS2 + k4[4]*gMNS + k5[4]) * r_gas;
  agas[2]  = (k1[5]*gMNS4 + k2[5]*gMNS3 + k3[5]*gMNS2 + k4[5]*gMNS + k5[5]) * r_gas;
  if(ThisTask==0)
    {
      printf("Adding acceleration due to star triple Miyamoto-Nagai disk with effective M = %g, Rd = %g, Zd = %g\n",NMmstar[0]+NMmstar[1]+NMmstar[2],(astar[0]+astar[1]+astar[2])/3,b_stars);
      printf("Adding acceleration due to gas  triple Miyamoto-Nagai disk with effective M = %g, Rd = %g, Zd = %g\n",NMmgas[0]+NMmgas[1]+NMmgas[2],(agas[0]+agas[1]+agas[2])/3,b_gas);
    }
  /* Acceleration due to Stellar Mass*/
  accel(NMmstar,astar,b_stars);
  /* Acceleration due to Gas Mass */
  accel(NMmgas,agas,b_gas);
  /* Acceleration due to Bulge */
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {if (P[i].Type != 5)                                          /* only add an acceleration to non-BH particles.  do NOT do min_dist_to_bh > 0 */
	{
	  double dp[3];
	  ...
	  for(k=0;k<3;k++) {dp[k] = -P[i].min_xyz_to_bh[k];}
#endif
	  r2 = dp[0]*dp[0]+dp[1]*dp[1]+dp[2]*dp[2], r = sqrt(r2), f = r+HQ_a, m = HQ_Mtot*(r/f)*(r/f);
	  for(k=0;k<3;k++) {P[i].GravAccel[k] += -All.G * m * dp[k]/(r2*r);}
	}
    }
}
...
void accel(double M[3],double a[3],double b){
  int i,k; double r2, Rterm, Rterm2, myfacR, Zterm; double dp[3];
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {if (P[i].Type != 5)                                          /* only add an acceleration to non-BH particles.  do NOT do min_dist_to_bh > 0 */
        {
      for(k=0;k<3;k++) {dp[k] = P[i].Pos[k];}
      ...
      for(k=0;k<3;k++) {
	r2 = dp[0]*dp[0] + dp[1]*dp[1];
    Zterm = sqrt(b*b + dp[2]*dp[2]); /* sqrt((Zdisk^2 + dZ^2); appears several times  */
    Rterm = a[k] + Zterm; Rterm2 = sqrt(r2 + Rterm*Rterm); Rterm2 = Rterm2*Rterm2*Rterm2;
    myfacR = -All.G * M[k] / Rterm2; /* has units s^-2, so  multiply by length to get accel.  no sign; handle that in min_xyz_to_bh */
    /* remember, min_xyz_to_bh = x_BH - myx => positive if x_BH > myx => acceleration is in positive x if x_BH > myx, which is correct (attractive) */
    P[i].GravAccel[0] += myfacR * dp[0]; P[i].GravAccel[1] += myfacR * dp[1];
    P[i].GravAccel[2] += myfacR * dp[2] * Rterm/Zterm; // this has units of:  M*L^3*M^-1*T^-2*L^2*L^-1*L^-3 = L/T^2 
      }
...
double MNmass(double m,double z,double r){
  ...
  double k5[6] = {1.9487, -1.3077, 0.2242, 2.0074, 4.4441, 0.7333};
  double *MN = malloc(k)
  MN[0] = (k1[0] + k2[0] + k3[0] + k4[0] + k5[0]) * m * z / r;
  MN[1] = (k1[1] + k2[1] + k3[1] + k4[1] + k5[1]) * m * z / r;
  MN[2] = (k1[2] + k2[2] + k3[2] + k4[2] + k5[2]) * m * z / r;
  return MN[3];
}
...
double MNa(double z){
  ...
  double k5[6] = {1.9487, -1.3077, 0.2242, 2.0074, 4.4441, 0.7333};
  double a[3];
  a[0]  = (k1[3] + k2[3] + k3[3] + k4[3] + k5[3]) * z;
  a[1]  = (k1[4] + k2[4] + k3[4] + k4[4] + k5[4]) * z;
  a[2]  = (k1[5] + k2[5] + k3[5] + k4[5] + k5[5]) * z;
  return a;
}
*/
